...
export function authRoutes(server: FastifyInstance) {
  ...
  server
    ...
    .post('/register', { schema: { body: authSchema } }, async (request, reply) => {
      try {
        ...
        const hashedPassword = scryptSync(password, salt, LOGIN_CONFIG.SCRYPT_KEY_LENGTH).toString(
          'hex',
        )
        ...
        reply.code(response.code === ErrorCode.USER_ALREADY_EXISTS ? 409 : 500).send(response)
      }
    })
  ...
  server
    .withTypeProvider<ZodTypeProvider>()
    .post('/login', { schema: { body: authSchema } }, async (request, reply) => {
      try {
