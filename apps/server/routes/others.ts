...
export function otherRoutes(server: FastifyInstance) {
  ...
  server.withTypeProvider<ZodTypeProvider>().post(
    ...
    async (request, reply) => {
      try {
        ...
        if (isAlreadyContact) {
          throw new ApiError(ErrorCode.DUPLICATE_ITEM, 'User is already in the list.', 409)
        }
  ...
  server.withTypeProvider<ZodTypeProvider>().post(
    ...
    async (request, reply) => {
      try {
        ...
        const response = handleError(error, server)
        const statusCode = response.code === ErrorCode.ITEM_NOT_FOUND ? 404 : 500
        reply.code(statusCode).send(response)
      }
    },
  )
  ...
  // Get others endpoint
  server.get('/get-others', { preHandler: [server.authenticate] }, async (request, reply) => {
    try {
      const username = request.username
